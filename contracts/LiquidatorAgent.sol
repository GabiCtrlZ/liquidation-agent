...
contract LiquidatorAgent is FlashLoanReceiverBase {
    ...
    function executeOperation(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            uint256 initialDebtBalance = IERC20(_debtAsset).balanceOf(
                address(this)
            );
            ...
            IERC20(_collateral).approve(
                address(swapper),
                IERC20(_collateral).balanceOf(address(this)) -
                    initialCollateralBalance // if the agent got some of the collateralAsset from before, no need to swap everything
            );
    ...
    function findUserCurrentReserveIndex(address user, uint256 reservesCount)
        external
        view
        returns (uint256)
    {
        ...
        for (
            ...
            if ((userConfig.data >> (reserveIndex * 2)) & 3 != 0) {
                // this is the calculation used in isUsingAsCollateral function
                return reserveIndex;
            }
        }
                ...
                return
                    DebtData(
                        IERC20(reserve.stableDebtTokenAddress).balanceOf(user) +
                            IERC20(reserve.variableDebtTokenAddress).balanceOf(
                                user
                            ),
                        reserves[i]
                    );
